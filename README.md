...
| **IAM & Pipeline** |  Clés d'accès AWS stockées dans les secrets GitHub | Sécurité |
...
|  2 | **Infrastructure**   | `infrastructure.yml` | Validation, scan tfsec et déploiement GitOps de l'IaC | TP4 |
...
### Étape 4 : Destruction complète de l'infrastructure (Pipeline Destroy)

Pour supprimer entièrement l'infrastructure et les ressources associées, un pipeline dédié **"💥 Destroy All"** a été mis en place :

| Élément | Description |
|---------|-------------|
| **Déclenchement** | Manuel (`workflow_dispatch`) via l'onglet Actions de GitHub. |
| **Confirmation obligatoire** | ⚠️ L'utilisateur doit taper **`CONFIRMER`** dans le champ de validation pour lancer la suppression, évitant ainsi toute suppression accidentelle. |
| **Ressources supprimées** | VPC, RDS, ECS, ALB, WAF, CloudFront, S3 (état Terraform), DynamoDB (verrouillage), Logs CloudWatch, ECR, Secrets Manager. |
| **Ordre de suppression** | 1. Infrastructure → 2. Backend (S3 + DynamoDB) → 3. Ressources restantes (logs, ECR, secrets, ECS). |


**Procédure :**

1. Rendez-vous sur l'onglet **Actions** de votre dépôt GitHub.
2. Sélectionnez le workflow **"💥 Destroy All - Suppression complète de l'infrastructure AWS"**.
3. Cliquez sur **Run workflow**.
4. Dans le champ de validation, tapez **`CONFIRMER`**.
5. Cliquez sur **Run workflow** pour lancer la suppression complète.

> ⚠️ **Attention :** Cette action est irréversible. Toutes les données (RDS, S3, Secrets, Logs) seront définitivement supprimées.
